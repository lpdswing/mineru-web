...
def test_settings_backend_maps_to_file_backend_without_losing_specific_value():
    ...
    assert SettingsBackendType.VLM_ENGINE.to_file_backend() == BackendType.VLM_ENGINE
    assert SettingsBackendType.VLM_HTTP_CLIENT.to_file_backend() == BackendType.VLM_HTTP_CLIENT
    assert SettingsBackendType.HYBRID_ENGINE.to_file_backend() == BackendType.HYBRID_ENGINE
...
def test_backend_normalization_keeps_mineru_prefix_backends_compatible():
    assert normalize_backend_value("vlm-auto-engine") == "vlm-engine"
    assert normalize_backend_value("hybrid-auto-engine") == "hybrid-engine"
